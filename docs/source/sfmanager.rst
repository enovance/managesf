...
To set a random login password for Gerrit in order to use tools like
https://github.com/stackforge/gertty, for the current user.
...
and to remove the user and password from Gerrit.
...
To configure the ssh keys in Gerrit as described in the deploy_ documentation.
...
and to remove the ssh key from gerrit
...
To initiate the test pipeline for a new project.
