...
from managesf.model.yamlbkd.engine import get_resources
...
class HooksController(RestController):
    ...
    def post(self, hook_name):
        ...
        tracker = None
        for _, project in resources.get('projects', {}).items():
            if change.get('project') in project.get('source-repositories', []):
