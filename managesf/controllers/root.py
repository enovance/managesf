...
class SSHConfigController(RestController):
    def __init__(self):
        self.confdir = None
        if getattr(conf, "sshconfig", None):
            self.confdir = conf.sshconfig.get('confdir')
            self.filename = os.path.join(self.confdir, "config")
            self.hostname = conf.sshconfig.get('hostname', 'gerrit.tests.dom')
            self.username = conf.sshconfig.get('username', 'gerrit')
            self.key_filename = conf.sshconfig.get(
                'key_filename', '/var/www/managesf/gerrit_ssh_key')
    ...
    def _dump_config(self, configdict):
        ...
        for alias, config in configdict.items():
            ...
            if identityfile_content:
                ...
                config['identityfile'] = identityfile_name
    ...
    def _copy2gerrit(self, filenames):
        ...
        for filename in filenames:
            ...
            try:
                sftp.put(filename, os.path.expanduser(remote_filename))
            except IOError:
                success = False
        sftp.close()
