...
class MembershipController(RestController):
    ...
    def put(self, project=None, user=None):
        ...
        try:
            ...
            # gerrit_controller.add_user_to_projectgroups(project, user, inp['groups'])
    ...
    def delete(self, project=None, user=None, group=None):
        ...
        try:
            ...
            # gerrit_controller.delete_user_from_projectgroups(project, user, group)
            ...
            for gn in ('ptl-group-members', 'core-group-members',
                ...
                for u in inp.get(gn, []):
                    if '@' not in u:
                        response.status = 400
                        return "User must be identified by its email address"
            #gerrit_controller.init_project(name, inp)
