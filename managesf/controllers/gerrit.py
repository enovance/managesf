...
class GerritRepo(object):
    ...
    def review_changes(self, commit_msg):
        ...
        cmd = "ssh-agent bash -c 'ssh-add %s; git review -s'" % conf.gerrit['sshkey_priv_path']
        self._exec(cmd, cwd=self.infos['localcopy_path'])
...
def propose_test_definition(project_name, requester):
    ...
    config_git.review_changes('%s proposes initial test definition for project %s' %
...
exit 0;'''
    ...
    project_git.review_changes('%s proposes initial test scripts for project %s' %
                              (requester, project_name))
