...
class TestGroupController(FunctionalTest):
    ...
    def test_create_group(self):
        ...
        with nested(*ctx) as (sgm, rgm):
            resp = self.app.put_json('/group/grp1',
                                     {'description': 'Nice dev team'},
                                     extra_environ=env,
                                     status="*")
            sgm.assert_called_with('grp1', 'Nice dev team')
            rgm.assert_called_with('grp1', 'Nice dev team')
    ...
    def test_update_group(self):
        env = {'REMOTE_USER': 'user1'}
        ctx = [patch.object(SFGerritGroupManager, 'update'),
               patch.object(RedmineGroupManager, 'update'),
               patch.object(SFGerritGroupManager, 'get'),
               patch.object(SFUserManager, 'get')]
        with nested(*ctx) as (sgm, rgm, sgg, sfum):
            sfum.return_value = {'email': 'user1@sftests.com'}
            sgg.return_value = {'grp1': [{'email': 'user2@sftests.com'}]}
            resp = self.app.post_json('/group/grp1',
                                      {'members': ['user1@sftests.com',
                                                   'user2@sftests.com']},
                                      extra_environ=env,
                                      status="*")
            sgm.assert_not_called()
            rgm.assert_not_called()
        # user is not part of the group
        # update is then forbidden
        self.assertEqual(resp.status_int, 403)

        with nested(*ctx) as (sgm, rgm, sgg, sfum):
            sfum.return_value = {'email': 'user1@sftests.com'}
            sgg.return_value = {'grp1': [{'email': 'user1@sftests.com'}]}
            members = {'members': ['user1@sftests.com',
                                   'user2@sftests.com']}
            resp = self.app.post_json('/group/grp1',
                                      members,
                                      extra_environ=env,
                                      status="*")
            sgm.assert_called_with('grp1', members['members'])
            rgm.assert_called_with('grp1', members['members'])
        # user is part of the group
        # update is then accepted
        self.assertEqual(resp.status_int, 204)

        with nested(*ctx) as (sgm, rgm, sgg, sfum):
            sfum.return_value = {'email': 'user1@sftests.com'}
            sgg.return_value = {'grp1': [{'email': 'user1@sftests.com'}]}
            # Redmine group update fail
            rgm.side_effect = self.exc3
            members = {'members': ['user1@sftests.com',
                                   'user2@sftests.com']}
            resp = self.app.post_json('/group/grp1',
                                      members,
            ...
            sgm.assert_called_with('grp1', members['members'])
            rgm.assert_called_with('grp1', members['members'])
        # The resource return a success as gerrit group update was
        # success
        self.assertEqual(resp.status_int, 204)
        ...
        with nested(*ctx) as (sgm, rgm, sgg, sfum):
            sfum.return_value = {'email': 'user1@sftests.com'}
            sgg.return_value = {'grp1': [{'email': 'user1@sftests.com'}]}
            # Gerrit group update fail
            sgm.side_effect = self.exc3
            members = {'members': ['user1@sftests.com',
                                   'user2@sftests.com']}
            resp = self.app.post_json('/group/grp1',
                                      members,
            ...
            sgm.assert_called_with('grp1', members['members'])
            rgm.assert_not_called()
        # The resource return a failure as gerrit group fails
