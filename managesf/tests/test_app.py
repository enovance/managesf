...
from managesf.services.nodepool.node import SFNodepoolSSHNodeManager as SFNNM
from managesf.services.nodepool.image import SFNodepoolSSHImageManager as SFNIM
...
class TestAgentsController(FunctionalTest):
    def test_get(self):
        with patch.object(SFGerritProjectManager, 'get_user_groups'):
            environ = {'REMOTE_USER': 'SF_SERVICE_USER'}
            with patch.object(SFNNM, 'get') as get:
                ...
                self.assertEqual(2,
                                 len(j))
                # with id
                resp = self.app.get('/agents/id/27',
                                    extra_environ=environ, status="*")
                ...
                self.assertEqual(2,
                                 len(j))
                # wrong id type
                resp = self.app.get('/agents/id/yeah_no',
                                    extra_environ=environ, status="*")
                self.assertEqual(400, resp.status_int)
        ...
        with patch.object(SFGerritProjectManager, 'get_user_groups'):
            ...
            with patch.object(SFNNM, 'get') as get, \
                ...
                get.return_value = [{'mock1': 'val1', },
                                    {'mock2': 'val2', }, ]
                # no filtering arg
                resp = self.app.put('/agents/id/123/',
                                    extra_environ=environ, status="*")
                self.assertEqual(200, resp.status_int)
                j = json.loads(resp.body)
                self.assertTrue('nodepool' in j.keys())
                ...
                # wrong id type
                resp = self.app.put('/agents/id/yeah_no',
                                    extra_environ=environ, status="*")
                self.assertEqual(400, resp.status_int)
        ...
        with patch.object(SFGerritProjectManager, 'get_user_groups'):
            ...
            with patch.object(SFNNM, 'get') as get, \
                ...
                get.return_value = [{'mock1': 'val1', },
                                    {'mock2': 'val2', }, ]
                # no filtering arg
                resp = self.app.delete('/agents/id/123/',
                                       extra_environ=environ, status="*")
                self.assertEqual(200, resp.status_int)
                j = json.loads(resp.body)
                self.assertTrue('nodepool' in j.keys())
    ...
    def test_errors(self):
        with patch.object(SFGerritProjectManager, 'get_user_groups'):
            ...
            with patch.object(SFNNM, 'get') as get, \
                ...
                resp = self.app.get('/agents/id/123/',
                                    extra_environ=environ, status="*")
                self.assertEqual(500, resp.status_int)
                j = json.loads(resp.body)
                self.assertEqual('Not a chance',
                                 j['error_description'])
                resp = self.app.post_json('/agents/id/123/authorize_key/',
                                          {'public_key': 'a', 'user': 'b'},
                                          extra_environ=environ, status="*")
                self.assertEqual(500, resp.status_int)
                j = json.loads(resp.body)
        ...
        with patch.object(SFGerritProjectManager, 'get_user_groups'):
            ...
            with patch.object(SFNNM, 'get') as get, \
                ...
                self.assertEqual({'nodepool': 'OK'},
                                 j)
                add.assert_called_with(123, 'a', 'b')
        ...
        with patch.object(SFGerritProjectManager, 'get_user_groups'):
            ...
            with patch.object(SFNIM, 'get') as get:
                get.return_value = [{'mock1': 'val1', },
                                    {'mock2': 'val2', }, ]
                # no filtering arg
                resp = self.app.get('/agents/image///',
                                    extra_environ=environ, status="*")
                self.assertEqual(200, resp.status_int)
                ...
                self.assertEqual(2,
                                 len(j))
                # with provider
                resp = self.app.get('/agents/image/blip//',
                                    extra_environ=environ, status="*")
                ...
                self.assertEqual(2,
                                 len(j))
                # with both
                resp = self.app.get('/agents/image/blip/blop/',
                                    extra_environ=environ, status="*")
        ...
        with patch.object(SFGerritProjectManager, 'get_user_groups'):
            ...
            with patch.object(SFNIM, 'update') as update:
                update.return_value = SlowStringIO('well that went OK' * 4096,
                                                   max_wait=10)
                resp = self.app.put('/agents/image/blip/blop/',
                                    extra_environ=environ, status="*")
