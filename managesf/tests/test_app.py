...
class TestResourcesController(FunctionalTest):
    ...
    def test_post(self):
        ...
        data = {'resources': {'projects': {}}}
        repo_path = self.prepare_repo(data)
        proposed_data = {'resources': {'projects': {
        ...
        with patch('managesf.controllers.root.conf') as conf:
            ...
            resp = self.app.post_json('/resources/',
                                      kwargs, status="*")
            self.assertIn(
                'Resource [type: projects, ID: id1] is going to be created.',
                resp.json)
            ...
            resp = self.app.post_json('/resources/',
                                      kwargs, status="*")
            self.assertIn(
                "Resource [type: projects, ID: id2] is going to be created.",
                resp.json)
            self.assertIn(
                "Resource [type: project, ID: idbogus] miss a mandatory key. "
