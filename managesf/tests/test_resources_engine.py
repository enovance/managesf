...
from managesf.model.yamlbkd.engine import SFResourceApplyException
...
class EngineTest(TestCase):
    ...
    def test_get_resources_priority(self):
        en = SFResourceBackendEngine(None,
                                     None)
        self.assertEqual(('dummies', 50),
                         en._get_resources_priority()[0])
        self.assertTrue(len(en._get_resources_priority()), 1)
    ...
    def test_apply(self):
        ...
        with nested(*patches) as (l, g, v):
            l.return_value = (None, None)
            v.side_effect = SFResourceApplyException('')
            engine = SFResourceBackendEngine(path, None)
            status, logs = engine.apply(None, None, None, None)
            self.assertEqual(len(logs), 1)
            self.assertFalse(status)
    ...
    def test_apply_changes(self):
        ...
        with patch('managesf.model.yamlbkd.resources.'
                   'dummy.DummyOps.create') as c:
            c.side_effect = SFResourceApplyException("Resource API error")
            changes = {'dummies': {'create': {'myprojectid': {}}}}
            self.assertRaises(SFResourceApplyException,
