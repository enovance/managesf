    ...
    def test_is_admin(self):
        patches = [patch.object(GerritUtils,
                   ...
                   patch.object(GerritUtils,
                                'get_group_id'), ]
        with nested(*patches) as (f, g):
            f.return_value = ('mock_group_1', 'mock_group_2')
            g.return_value = 'mock_group_1'
            self.assertEqual(True,
                             self.gerrit.role.is_admin('mockadmin'))
        with nested(*patches) as (f, g):
            f.return_value = ('mock_group_1', 'mock_group_2')
            g.return_value = 'mock_group_3'
...
class TestSFGerritRepositoryManager(BaseSFGerritService):
    def test_create(self):
        patches = [patch.object(GerritUtils,
                   ...
                   patch.object(utils.GerritRepo,
                                'push_master')]
        def ggi(prj): return {'ore': 1, 'ptl': 2, 'dev': 3}.get(prj[-3:], 0)
        with nested(*patches) as (get_group_id,
            ...
            self.gerrit.repository.create('proj', 'proj description',
                                          None,
                                          False)
            config_paths = {'project.config': '''[project]
...
class TestSFGerritMembershipManager(BaseSFGerritService):
    ...
    def test_create(self):
        ...
        with nested(*patches) as (get_my_groups,
                                  get_group_id,
                                  is_admin,
                                  add_group_member,
                                  group_exists):
            get_my_groups.return_value = ('ptl_gid',)
        ...
        with nested(*patches) as (get_my_groups,
                                  ...
                                  add_group_member,
                                  group_exists):
            get_my_groups.return_value = ('core_gid',)
            get_group_id.side_effect = ggi_side_effect
            is_admin.return_value = False
            group_exists.return_value = True
            self.gerrit.membership.create('requestor',
