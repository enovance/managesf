...
class YAMLBackendTest(TestCase):
    ...
    def prepare_base_git_repo(self):
        db_path = tempfile.mkdtemp()
        self.db_path.append(db_path)
        repo = git.Git(db_path)
        repo.init()
        # For unittest the origin is itself. In a real context
        # origin should be the gerrit/config repo
        repo.execute(['git', 'remote', 'add',
                      'origin', 'file://%s' % db_path])
        ...
        data2 = {'resources': {'projects': {
                 'SF/managesf': {
                     'git-repo': 'managesf',
                     'description': 'REST server for SF',
                     'gitweb': 'http://gitweb.com/',
                 }}}}
        ...
        with open(os.path.join(db_path, '2.yaml'), 'w') as dbfile:
            yaml.safe_dump(data2,
                           dbfile,
                           allow_unicode=True,
                           default_flow_style=False)
        repo.execute(['git', 'add', '1.yaml', '2.yaml'])
    ...
    def test_git_dir_init(self):
        ...
        db_path = self.prepare_base_git_repo()
        # Init the YAML DB
        db = yamlbackend.YAMLBackend(db_path)
        # Check the content has been loaded
        self.assertIn("SF/software-factory",
                      db.get_leafs()['resources']['projects'])
        self.assertIn("SF/managesf",
                      db.get_leafs()['resources']['projects'])
    ...
    def test_git_dir_get_callbacks_ops(self):
        # Prepare a GIT repo with content
        db_path = self.prepare_base_git_repo()
        repo = git.Git(db_path)
        data = {'resources': {'projects': {
                'SF/cauth': {
                    'git-repo': 'cauth',
                    'description': 'AUTH server for SF',
                    'gitweb': 'http://gitweb.com/',
                }}}}
        with open(os.path.join(db_path, '3.yaml'), 'w') as dbfile:
            yaml.safe_dump(data,
                           dbfile,
                           allow_unicode=True,
                           default_flow_style=False)
        repo.execute(['git', 'add', '3.yaml'])
        repo.update_environment(GIT_AUTHOR_EMAIL='test@test.com')
        repo.update_environment(GIT_AUTHOR_NAME='test')
        repo.update_environment(GIT_COMMITTER_EMAIL='test@test.com')
        repo.update_environment(GIT_COMMITTER_NAME='test')
        repo.execute(['git', 'commit', '-m', 'cmt2'])
        db = yamlbackend.YAMLBackend(db_path)
        ops = db.get_ops_from_head_1()
