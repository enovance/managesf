...
class TestMembershipAction(BaseFunctionalTest):
    def test_project_add_user_to_groups(self):
        args = self.default_args
        cmd = 'membership add --user u1 --project p1 --groups dev-group'.split()
        args += cmd
...
class TestReplicationActions(BaseFunctionalTest):
    ...
    def test_configure_add(self):
        args = self.default_args
        args += 'replication configure add --section AAA timeout bbb'.split()
    ...
    def test_configure_remove(self):
        ...
        self.assert_secure('delete', args, cli.replication_action, expected_url)
    ...
    def test_replace_all(self):
        return
        args = self.default_args
        args += 'replication configure replace-all --section BBB url DDD'.split()
    ...
    def test_restore(self):
        args = self.default_args
        args += 'replication restore --filename /tmp/toto'.split()
