...
def project_command(sp):
    ...
    sp.add_parser('list_active_users')
...
def membership_action(args, base_url, headers):
    ...
    if args.subcommand == 'add':
        ...
        if args.groups:
            # a hack until I fix the SF test (the groups should be separated
            # by a space instead of a comma
            if len(args.groups) == 1 and \
               not isinstance(args.groups, basestring):
                args.groups = args.groups[0].split(',')
...
def project_action(args, base_url, headers):
    if args.command != 'project' and \
       args.subcommand not in ['delete', 'create', 'list_active_users']:
        return False
    ...
    elif args.subcommand == 'list_active_users':
        logger.info('List users assigned to projects')
        url = build_url(base_url, 'project/membership')
