...
def membership_action(args, base_url, headers):
    ...
    if args.subcommand == 'remove':
        logger.info('Remove member %s from project %s', args.user, args.project)
...
def replication_action(args, base_url, headers):
    if args.command not in ['restore', 'replication_config',
                            'trigger_replication', 'replication']:
        return False

    if args.command in ['restore', 'replication_config', 'trigger_replication']:
        logger.info('Deprecated syntax, please look at the replication command')
        args.subcommand = args.command
    ...
    if args.subcommand in ['replication_config', 'configure']:
    ...
    elif args.subcommand in ['trigger_replication', 'trigger']:
        headers['Content-Type'] = 'application/json'
        url = build_url(base_url, 'replication')
        info = {}
        if args.wait:
