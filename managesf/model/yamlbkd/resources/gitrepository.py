...
class GitRepositoryOps(object):
    ...
    def get_all(self):
        ...
        for name in ret:
            ...
            acl = file(acl_path).read()
            m = hashlib.md5()
            m.update(acl)
            acl_id = m.hexdigest()
            gitrepos[name]['name'] = name
            gitrepos[name]['acls'] = acl_id
            acls[acl_id] = {}
            acls[acl_id]['file'] = acl
            acls[acl_id]['groups'] = set()
