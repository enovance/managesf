...
class StoryboardOps(object):
    ...
    def update_project_groups(self, **kwargs):
        ...
        self._set_client()
        pg = self.client.project_groups.get_all(name=name)
        if not pg:
            # Create the project group
            self.client.project_groups.create(
        ...
        for sr_name in sources_repositories:
            ...
            if not sr['local-tracker'] == 'SFStoryboard':
                continue
            project = self.client.projects.get_all(name=sr_name)
            if project:
               wanted_included.append(project.id) 
