...
class Dummy(BaseResource):
    ...
    PRIORITY = 50
    CALLBACKS = {
        'update': lambda conf, new, kwargs: DummyOps(conf, new).update(**kwargs),
        'create': lambda conf, new, kwargs: DummyOps(conf, new).create(**kwargs),
        'delete': lambda conf, kwargs: DummyOps(conf, new).delete(**kwargs),
