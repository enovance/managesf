...
class YAMLBackend(object):
    ...
    def _load_db(self):
        ...
        for f in yamlfiles:
            ...
            # TODO: Verify at YAML load time that duplicated resources key
            ...
            # loading malicious yaml serialized object.
            # So a reviewer should take care to the duplicated keys issue and
            # also look at the logs for checking not expected detected changes.
            #
            # Nevertheless between two or more yaml files this check can be
            # implemented here. Also adding a yaml linter call in the Jenkins
            # jobs can, maybe, detect the duplicate key issue.
            try:
    ...
    def _validate_base_struct(self, data):
        ...
        try:
            for rtype, resources in data['resources'].items():
                for rid, resource in data['resources'][rtype].items():
