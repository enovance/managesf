    ...
    def _get_data_diff(self, prev, new):
        ...
        for ctype, elm in changes.items():
            for path, value in elm.items():
                ...
                elif ctype == 'iterable_item_removed':
                    if 'delete' not in sanitized_changes[rtype]:
                        sanitized_changes[rtype]['delete'] = []
                    sanitized_changes[rtype]['delete'].append(value)
                elif ctype == 'values_changed':
                    if 'update' not in sanitized_changes[rtype]:
                        sanitized_changes[rtype]['update'] = {}
                    key_changed = re.search(".*\['(.*)'\]$", path)
                    key_changed = key_changed.groups()[0]
                    path = path.replace('root', 'new')
                    path = re.sub("\['([a-z]|[A-Z]|[0-9])+'\]$", '', path)
                    data = eval(path)
                    if not data['id'] in sanitized_changes[rtype]['update']:
                        rchange = {'data': data,
                                   'key_changed': set([key_changed])}
                        sanitized_changes[rtype]['update'][
                            data['id']] = rchange
                    else:
                        sanitized_changes[rtype]['update'][
                            data['id']]['key_changed'].add(key_changed)
                else:
