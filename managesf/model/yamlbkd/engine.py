...
class SFResourceApplyException(Exception):
...
class SFResourceBackendEngine(object):
    ...
    def _get_update_change(self, rtype, prev, new, rids):
        ...
        for rid in rids:
            ...
            if not changes:
                continue
            r_key_changes[rid] = {'data': new_data, 'changed': []}
                    ...
                    try:
                        apply_logs.append(
                            "Resource [type: %s, ID: %s] will be %s." % (
                                rtype, rid, ctype + 'd'))
                        if ctype == 'update':
                            MAPPING[rtype].CALLBACKS[ctype](conf, data['data'])
                        else:
                            r = MAPPING[rtype](rid, data)
                            r.set_defaults()
                            _data = r.get_resource()
                            MAPPING[rtype].CALLBACKS[ctype](conf, _data)
                        apply_logs.append(
                        ...
                        apply_logs.append(
                            "Resource [type: %s, ID: %s] %s op failed." % (
                                rtype, rid, ctype))
                        raise SFResourceApplyException(str(e))
        return apply_logs
