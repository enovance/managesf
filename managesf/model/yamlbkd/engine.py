        ...
        for rid in rids:
            ...
            for ctype, _changes in changes.items():
                if ctype in ('values_changed', 'iterable_item_removed',
                             'iterable_item_added'):
                    for c in _changes:
                        # TODO(fbo): Be sure all ids follow this regex
                        # Also when we will fetch exisiting data, make
                        # sure the id we set is according the regex too
            ...
            for rid, data in resources.items():
                ...
                for deps_type, deps_ids in deps.items():
                    if (deps_type in u_resources and
                        ...
                        logs.append(
                            "Resource [type: %s, ID: %s] need a refresh as "
                            "at least one of its dependencies has been "
                            "updated" % (rtype, rid))
                    sanitized_changes.setdefault(rtype, {'update': {}})
                    sanitized_changes[rtype]['update'][rid] = {'data': data}
        return logs
