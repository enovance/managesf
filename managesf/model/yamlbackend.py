...
class YAMLBackend(object):
    ...
    def load_db_from_directory(self):
        subdir = '.'
        yamlfiles = [f for f in
                     os.listdir(
                         os.path.join(subdir,
                                      self.db_path)) if f.endswith('.yaml')]
        self.data = None
        for f in yamlfiles:
            logger.info("Reading %s ..." % f)
            if not self.data:
                self.data = yaml.load(
                    file(os.path.join(subdir, self.db_path, f)))
            else:
                data_to_append = yaml.load(
                    file(os.path.join(subdir, self.db_path, f)))
                for rtype, resources in data_to_append['resources'].items():
                    if rtype in self.data['resources']:
                        for r, v in resources.items():
                            if r in self.data['resources'][rtype].keys():
                                logger.warning("Duplicate resource %s:%s" % (
                                    rtype, r))
                            self.data['resources'][rtype][r] = v

    def load_db(self, directory=False):
        if directory:
            self.load_db_from_directory()
