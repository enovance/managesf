...
class SFRedmineMembershipManager(RedmineMembershipManager):
    ...
    def create(self, username, project, groups, user_is_owner=False):
        ...
        roles = self.plugin.role.get(username, project)
        logger.info("[%s] Add user %s in groups %s from project %s" %
    ...
    def update_membership(self, membership_id, role_ids):
        rm = self.plugin.get_client()
        return rm.update_membership(self, membership_id, role_ids)

    def update_project_membership(self, project_name, memberships):
        rm = self.plugin.get_client()
        return rm.update_project_membership(self, project_name, memberships)

    def delete(self, username, project_name, group):
        ...
        if not m:
            return None
        user_roles = self.plugin.role.get(username, project_name)
        if group is None:
            # delete every group
            if ((not self.plugin.role.is_admin(username))
                    and ('Manager' not in user_roles)):
                msg = "[%s] Aborted because %s is not admin or Manager"
                logger.debug(msg % (self.plugin.service_name, username))
                raise exc.Unauthorized(msg % (self.plugin.service_name,
                                              username))
