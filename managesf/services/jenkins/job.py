...
class SFJenkinsJobManager(base.JobManager):
    ...
    def get_job(self, job_name, job_id=None,
        ...
        if job_id:
            job = self.get_job_status(job_name, job_id)
            return [job, ]
        # use some filtering
        filter = {"JOB_NAME": job_name, }
        filter_conditions = []
        if change:
            filter_conditions.append('change == "%s"' % change)
        if patchset:
            filter_conditions.append('patchset == "%s"' % patchset)
        filter["FILTER_CONDITION"] = " && ".join(filter_conditions)
        client = self.plugin.get_client()
        results = client.run_script(GROOVY_JOB_FILTER % filter)
    ...
    def get_job_parameters(self, job_name, job_id):
        ...
        client = self.plugin.get_client()
        try:
            actions = client.get_build_info(job_name, job_id)['actions']
            params = {}
            for a in actions:
                if a.get('parameters'):
                    params = a
    ...
    def get_job_status(self, job_name, job_id):
        ...
        status = {'job_name': job_name,
                  ...
                  'status': None}
        try:
            build_info = client.get_build_info(job_name, job_id)
    ...
    def get_job_logs(self, job_name, job_id):
        ...
        try:
            ...
            if status['logs_url']:
                logs_suffix = "timestamps/?time=HH:mm:ss.S&appendLog"
    ...
    def run(self, job_name, job_parameters=None):
        ...
        status = {'job_name': job_name,
                  ...
                  'status': None}
        try:
            build_info = client.get_build_info(job_name, next_build)
            if build_info.get('building'):
                status['status'] = 'IN_PROGRESS'
    ...
    def stop(self, job_name, job_id):
        """stop a running job"""
        client = self.plugin.get_client()
        client.stop_build(job_name, job_id)
        msg = u'[%s] job stopped manually: %s/%s'
        logger.debug(msg % (self.plugin.service_name,
                            job_name, job_id))
        return self.get_job_status(job_name, job_id)
